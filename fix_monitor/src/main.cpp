...
namespace {
    ...
    // Runtime state plus the configuration that decides how to read it.
    //
    // The two belong in one document on purpose. "expected 5, received 1" means
    // opposite things under reset_each_logon and under persistent, so anything
    // that answers questions about a session needs both halves at once; handing
    // out the counters alone invites a confident wrong answer.
    std::string sessions_json(const SessionRegistry &reg,
                              const std::vector<SessionConfig> &configured) {
        ...
        for (const auto &s: snaps) {
            ...
            auto cfg_it = std::find_if(configured.begin(), configured.end(),
                                       [&](const SessionConfig &c) {
                                           return c.session_id() == s.session_id;
                                       });
            ...
            if (cfg_it != configured.end()) {
                const SessionConfig &c = *cfg_it;
                os << ",\"config\":{"
                        << "\"connection_type\":\""
                        << (c.connection_type.empty() ? "unknown" : c.connection_type) << "\","
                        << "\"heartbeat_interval\":" << c.heartbeat_interval << ","
                        << "\"source\":\"" << (c.from_quickfix ? "quickfix_config" : "fixmon_ini")
                        << "\","
                        // The raw four, so a reader can check the derivation
                        // rather than take it on trust.
                        << "\"reset_on_logon\":\"" << to_string(c.reset_on_logon) << "\","
                        << "\"reset_on_logout\":\"" << to_string(c.reset_on_logout) << "\","
                        << "\"reset_on_disconnect\":\"" << to_string(c.reset_on_disconnect) << "\","
                        << "\"persist_messages\":\"" << to_string(c.persist_messages) << "\","
                        // ...and what they add up to, so it is not re-derived
                        // slightly differently by every reader.
                        << "\"seq_reset_policy\":\"" << c.seq_reset_policy() << "\","
                        << "\"resend_capability\":\"" << c.resend_capability() << "\""
                        << "}";
            }
            os << "}";
        }
    ...
    void publish_state(MetricRegistry &reg, const SessionRegistry &sessions, int64_t now) {
        for (const auto &s: sessions.snapshot()) {
            ...
            // Signed on purpose. Negative means the engine stamped a message
            // in our future, which is a clock problem and not a lag problem,
            // and an absolute value would erase exactly that distinction.
            set(reg, "fixmon_engine_clock_skew_seconds", l,
                static_cast<double>(s.clock_skew_ns) / 1e9);
        }
    }
        ...
        set(reg, "fixmon_session_info",
            {
                ...
                // The derived pair rather than the raw four: this series exists
                // to be joined against the sequence-gap counters, and what that
                // join needs to know is whether a gap is expected at all. Still
                // one series per session, so the cardinality is unchanged.
                {"seq_reset_policy", sc.seq_reset_policy()},
                {"resend_capability", sc.resend_capability()}
            },
            1.0);
    ...
    void print_session(const SessionConfig &sc, int adapters) {
        ...
        std::cout << "    sequence: " << sc.seq_reset_policy()
                << ", resend " << sc.resend_capability() << "\n";
        if (std::string(sc.seq_reset_policy()) == "unknown") {
            // Worth a line of its own. Everything else about this session will
            // look complete, and the gap only shows up later as an explanation
            // that sounds right and is not.
            std::cout << "      (ResetOnLogon/ResetOnLogout/ResetOnDisconnect not known - "
                    "a sequence gap cannot be judged without them)\n";
        }
    ...
    // What this collector will and will not keep. Printed on both paths: a
    // retention or masking setting that only becomes visible once you are
    // already collecting is one people get wrong and find out about later.
    void print_data_policy(const AppConfig &cfg) {
        std::cout << "masking: message bodies "
                << (cfg.mask_message_bodies ? "masked" : "STORED IN FULL")
                << ", credential tags always masked\n";
        std::cout << "retention: "
                << (cfg.retention_days > 0
                        ? std::to_string(cfg.retention_days) + " days"
                        : std::string("disabled - the store will grow without bound"))
                << "\n";
    }
...
int main(int argc, char **argv) {
    ...
    try {
        cfg = ini_path.empty()
                  ? config_from_quickfix(quickfix_paths)
    ...
    // Back-pressure rather than loss. try_push leaves the value untouched when
    // the queue is full (it only moves out of it after the slot is claimed), so
    // retrying the same event is safe. While an adapter sleeps here it also
    // stops reading its log, and the unread lines simply wait on disk - which is
    // the whole reason this is a tailer and not a packet capture.
    //
    // The bound still matters: without it a stalled consumer would hang the
    // adapters forever, and shutdown would deadlock. Past the bound we drop and
    // count, exactly as before.
    const int wait_ms = cfg.queue_full_wait_ms;
    auto sink = [&queue, &dropped, &queue_waits, wait_ms](Event &&ev) {
        if (queue.try_push(std::move(ev))) return;

        queue_waits.fetch_add(1, std::memory_order_relaxed);
    ...
    for (const auto &sc: cfg.sessions) {
        ...
        if (!sc.message_log_path.empty()) {
            adapters.push_back(std::make_unique<MessageLogAdapter>(
                sc, cfg.from_beginning, cfg.poll_interval_ms, cfg.mask_message_bodies));
            ++attached;
        }
    ...
    HttpServer http(cfg.metrics_port,
                    ...
                    [&sessions, &cfg] { return sessions_json(sessions, cfg.sessions); });
    if (!http.start(err)) {
        std::cerr << "http: " << err << "\n";
        store.close();
        return 1;
    ...
    while (!g_stop.load(std::memory_order_relaxed)) {
        ...
        if (std::chrono::duration_cast<std::chrono::milliseconds>(now_tp - last_flush).count()
            ...
            mirror(registry, "fixmon_queue_full_waits_total", {}, queue_waits.load());
            ...
            mirror(registry, "fixmon_store_rows_purged_total", {}, store.rows_purged());
        ...
        // Retention. Hourly rather than on every pass: the cutoff moves in days,
        // so anything more frequent is a write lock taken for nothing. It runs
        // on this thread on purpose - the same thread that stages rows is the
        // one that removes them, so a purge can never overlap a flush.
        if (cfg.retention_days > 0 &&
            std::chrono::duration_cast<std::chrono::minutes>(now_tp - last_purge).count() >= 60) {
            last_purge = now_tp;

            const int64_t cutoff =
                    now_ns() - static_cast<int64_t>(cfg.retention_days) * 86400LL * 1000000000LL;

            std::string perr;
            const size_t removed = store.purge_before(cutoff, perr);
            if (!perr.empty()) {
                // Loud, because a retention policy that quietly stops running
                // looks identical to one that has nothing left to delete.
                std::cerr << "retention: purge failed: " << perr << "\n";
            } else if (removed > 0) {
                std::cout << "retention: removed " << removed << " rows older than "
