...
def main():
    ...
    while time.time() < deadline:
        ...
        if elapsed > 24 and "logonrej" not in injected:
            injected.add("logonrej")
            evts.line("Disconnecting: Socket exception")
            evts.line("Attempting to reconnect in 5 seconds")
            evts.line("Initiated logon request")
            msgs.line(fix(out_seq, "A", True, **{"98": 0, "108": 30})); out_seq += 1
            msgs.line(fix(in_seq, "5", False, **{"58": "Invalid credentials"})); in_seq += 1
            evts.line("Logon rejected: Invalid credentials for BROKER1")
            print("[gen] injected logon reject")

        if elapsed > 27 and "unknown" not in injected:
